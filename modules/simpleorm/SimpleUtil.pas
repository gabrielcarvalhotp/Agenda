...
{$ENDIF}
  public
    ...
    class procedure DataSetToObjectList<T: TSimpleEntity, constructor>
      (const poDataSet: TDataSet; const poLista: TSimpleEntityList<T>);
      overload;
    class procedure GetValuesFromDataset(const poDataSet: TDataSet;
      const poClasse: TObject);
{$IFNDEF CONSOLE}
    class procedure GetObjectFromForm(const aForm: TForm;
      const aObject: TObject);
    class procedure SetFormFromObject(const aForm: TForm;
      const aObject: TObject);
{$ENDIF}
  end;
...
{ TcdRTTIUtils }
uses
  System.Rtti, SysUtils, FireDAC.Comp.Client, SimpleRTTIHelper,
  SimpleAttributes,
  LocalCache4D;

class procedure TSimpleUtil.DataSetToObjectList<T>(const poDataSet: TDataSet;
  const poLista: TObjectList<T>);
var
  oObjeto: T;
begin
...
begin
  ...
  begin
    if (aComponent as TEdit).NumbersOnly and ((aComponent as TEdit).Text = '')
    then
      Exit('0')
    else
